...
def main_worker(gpu, ngpus_per_node, args):
    ...
    loss_fn = losses.ContrastiveLoss(distance=CosineSimilarity(), pos_margin=args.pos_margin, neg_margin=args.neg_margin)
...
def create_labels(num_pos_pairs, previous_max_label, args):
    # create labels that indicate what the positive pairs are
    labels = torch.arange(0, num_pos_pairs)
    labels = torch.cat((labels, labels)).cuda(args.gpu, non_blocking=True)
...
def train_one_epoch(train_loader, model, loss_fn, optimizer, scaler, epoch, args):
    ...
    for images, _ in progress:
        ...
        labels = create_labels(batch_size, previous_max_label, args)
